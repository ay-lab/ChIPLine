...
usage: ./IDR_SubSampleBAM_Main.sh [-h] [-I inpfile1.bam] [-I inpfile2.bam] [-d OutDir] [-P PathIDRCode] [-n 'IDR_test'] [-c 25000] [-T 0/1] [-C control.bam]
...
while getopts "I:n:d:c:T:C:P:" opt;
do
	case "$opt" in
		I) InpFile+=($OPTARG);;
		n) PREFIX=$OPTARG;;
...
do
	...
	do
		...
		if [[ ! -z $CONTROLBAM ]]; then
			./IDR_SubSampleBAM.sh -A $sample1 -B $sample2 -d $OutDir'/'$i'_and_'$j -P $dir2 -n $PREFIX -c $CountPeak -T $Tagmentation -C $CONTROLBAM
		else
			./IDR_SubSampleBAM.sh -A $sample1 -B $sample2 -d $OutDir'/'$i'_and_'$j -P $dir2 -n $PREFIX -c $CountPeak -T $Tagmentation
		fi
	done
done
...
if [ $Tagmentation == 1 ]; then
	plotfilename=$plotfilename'_T1'
else
	plotfilename=$plotfilename'_T0'
fi
