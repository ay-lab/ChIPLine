...
usage: ./IDR_SubSampleBAM.sh [-h] [-A BamFile1] [-B BamFile2] [-d OutDir] [-n 'IDR_test'] [-P PathIDRCode] [-c 25000] [-T 0/1] [-C control.bam]
Example:
...
Options:    
  ...
  -- required:
	...
	-n 	PREFIX 			 Prefix of output files. Default 'IDR_ChIP'.
	-c  CountPeak		 No of peaks in both replicates that will be compared. Default 25000.
	-T 	Tagmentation	 Binary variable. If 1, the input is a ChiPMentation data where the TAG Align files are created by 
...
while getopts "A:B:n:d:c:T:C:P:" opt;
do
	case "$opt" in
		A) BamFile1=$OPTARG;;
		B) BamFile2=$OPTARG;;
...
if [[ $readcount1 -gt $readcount2 ]]; then
	...
	if [ ! -f $TagAlignFile1 ]; then
		$TagAlignExec -I $OutDir'/temp_1.bam' -N `expr 1 - $Tagmentation` -O $TagAlignFile1
	fi
	if [ ! -f $TagAlignFile2 ]; then
		$TagAlignExec -I $BamFile2 -N `expr 1 - $Tagmentation` -O $TagAlignFile2
	fi
else
	...
	if [ ! -f $TagAlignFile2 ]; then
		$TagAlignExec -I $OutDir'/temp_2.bam' -N `expr 1 - $Tagmentation` -O $TagAlignFile2
	fi
	if [ ! -f $TagAlignFile1 ]; then
		$TagAlignExec -I $BamFile1 -N `expr 1 - $Tagmentation` -O $TagAlignFile1
	fi
fi
...
# the output folder name is like MACS2_0/1_C_T 
# (where 0/1 indicates first or second sample)
# _C and _T are optional and included only when corresponding options are provided as the input
...
if [ $Tagmentation == 1 ]; then
	MACS2_outdir1=$MACS2_outdir1'_T'
	MACS2_outdir2=$MACS2_outdir2'_T'
fi
...
if [ ! -f $MACS2PeakOutFile1 ]; then
	MACS2_cmd='macs2 callpeak -t '$TagAlignFile1' -f BED -n '$PREFIX'.macs2 --outdir '$MACS2_outdir1
	if [ $Tagmentation == 1 ]; then
...
if [ ! -f $MACS2PeakOutFile2 ]; then
	MACS2_cmd='macs2 callpeak -t '$TagAlignFile2' -f BED -n '$PREFIX'.macs2 --outdir '$MACS2_outdir2
	if [ $Tagmentation == 1 ]; then
		# for the ChIPMentation data, bandwidth of 200 is used
		MACS2_cmd=$MACS2_cmd' --bw 200'
	fi
...
# depending on the presence of control and Tagmentation parameters
# the folders will vary
# the folders have the following format: C(0/1)_T(0/1) depending on the input options
...
if [ $Tagmentation == 1 ]; then
	IDR_OutDir=$IDR_OutDir'_T1'
else
	IDR_OutDir=$IDR_OutDir'_T0'
fi
